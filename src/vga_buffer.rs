...
// color code structs combines foreground and background colors into a single u8
...
// Buffer struct represents the VGA buffer
#[repr(transparent)]
struct Buffer {
...
impl Writer {
    pub fn write_string(&mut self, s: &str) {
        for byte in s.bytes() {
            match byte {
                //check if the character is printable or newline
                0x20..=0x7e | b'\n' => self.write_byte(byte),
                //otherwise print a square character
...
//use lazy static with a spinlock
lazy_static! {
...
fn test_println_simple() {
    //meant to finish without panicking
